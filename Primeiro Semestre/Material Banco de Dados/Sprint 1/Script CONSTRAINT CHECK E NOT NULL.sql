...
-- CONSTRAINT: REGRA, CONFIGURAÇÃO OU RESTRIÇÃO DE UM CAMPO
	-- NOT NULL
    -- CHECK
    -- PK
...
ALTER TABLE revista MODIFY COLUMN nome varchar(40) not null;

SELECT * FROM revista;

ALTER TABLE revista ADD constraint chkCategoria
	CHECK (categoria IN ('Notícia', 'Diversos'));
    
UPDATE revista SET categoria = 'Infanto-juvenil'
	WHERE idRevista = 1;
-- Check constraint 'chkCategoria' is violated.
...
CREATE TABLE novela (
idNovela int primary key auto_increment,
nome varchar(30) not null,
tema varchar(40), 
constraint chkTema check (tema IN ('Amor', 'Humor'))
)auto_increment = 200;